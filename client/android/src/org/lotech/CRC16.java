...
public class CRC16 { 
	...
	/**
	  * Following is my port of some C code to Java.
	  */

	static final int CRC16_POLY = 0x8005;

	static short brev16( int in ) {
		int res = 0;
		for(int i = 0x8000, j = 0x0001; i != 0; i >>=1, j <<= 1 )
			if( ( i & in ) != 0 ) res |= j;
		return (short)(0x0000FFFF & res );
	}

	public static int calc2( byte[] bytes, int len ) {
		...
		while( i < len /*size > 0*/ ) {
			...
			/* Get next bit: */
			out <<= 1;
			out |= (bytes[i] /* *data*/ >> bits_read) & 1; // item a) work from the least significant bits
    ...
    public static void main( String[] args ) {
		...
		int crc = calc2( buf, buf.length );
		System.out.printf( "CRC16(%s) = %04X(%d)\n", args[0], crc, buf.length );
    }
}
